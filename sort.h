...
#undef SORT_TYPE_CPY
#define SORT_TYPE_CPY(dst, src, size) \
    memcpy((dst), (src), (size) * sizeof(SORT_TYPE))
#undef SORT_TYPE_MOVE
#define SORT_TYPE_MOVE(dst, src, size) \
    memmove((dst), (src), (size) * sizeof(SORT_TYPE))
...
#else
    return (SORT_TYPE *) kmalloc(size * sizeof(SORT_TYPE), GFP_KERNEL);
#endif
}
...
void MERGE_SORT_IN_PLACE_FRONTMERGE(SORT_TYPE *dst1,
                                    size_t l1,
                                    SORT_TYPE *dst2,
                                    size_t l2)
{
...
size_t MERGE_SORT_IN_PLACE_BACKMERGE(SORT_TYPE *dst1,
                                     size_t l1,
                                     SORT_TYPE *dst2,
                                     size_t l2)
{
...
void MERGE_SORT_IN_PLACE_RMERGE(SORT_TYPE *dst, size_t len, size_t lp, size_t r)
{
    ...
    for (i = 0; i < len; i += r) {
        ...
        if (q != i) {
            MERGE_SORT_IN_PLACE_ASWAP(dst + i, dst + q,
                                      r); /* swap it with current position */
    ...
    for (p = 2; p <= lr; p += 2) {
        ...
        if (q < m) {
            ...
            for (;;) {
                ...
                p0 = len - q1;
                MERGE_SORT_IN_PLACE_FRONTMERGE(dst + (p - q1), q, dst + p0 + q,
                                               q);
                q = q1;
            }
...
static __inline size_t QUICK_SORT_PARTITION(SORT_TYPE *dst,
                                            const size_t left,
                                            const size_t right,
                                            const size_t pivot)
{
...
static __inline size_t QUICK_SORT_HOARE_PARTITION(SORT_TYPE *dst, const size_t
l, const size_t r, const size_t pivot) { SORT_TYPE value; size_t i = l + 1;
  size_t j = r;
  if (pivot != l) {
...
static __inline size_t MEDIAN(const SORT_TYPE *dst,
                              const size_t a,
                              const size_t b,
                              const size_t c)
{
...
static void QUICK_SORT_RECURSIVE(SORT_TYPE *dst,
                                 const size_t original_left,
                                 const size_t original_right)
{
    ...
    while (1) {
        ...
        pivot = MEDIAN((const SORT_TYPE *) dst, left + ((middle - left) >> 1),
                       pivot, middle + ((right - middle) >> 1));
...
static void TIM_SORT_RESIZE(TEMP_STORAGE_T *store, const size_t new_size)
{
    if ((store->storage == NULL) || (store->alloc < new_size)) {
        SORT_TYPE *tempstore = (SORT_TYPE *) krealloc(
            store->storage, new_size * sizeof(SORT_TYPE), GFP_KERNEL);
                             ...
                             const size_t size)
{
    while (1) {
        ...
        if (stack_curr >= 4) {
            size_t A = stack[stack_curr - 4].length;
            ABC = (A <= B + C);
        } else {
            ABC = 0;
        }
...
void TIM_SORT(SORT_TYPE *dst, const size_t size)
{
    ...
    TIM_SORT_RUN_T *run_stack =
        kmalloc(sizeof(TIM_SORT_RUN_T) * TIM_SORT_STACK_SIZE, GFP_KERNEL);
    size_t stack_curr = 0;
    size_t curr = 0;
    ...
    while (1) {
        if (!CHECK_INVARIANT(run_stack, (int) stack_curr)) {
            stack_curr = TIM_SORT_COLLAPSE(dst, run_stack, (int) stack_curr,
                                           store, size);
            continue;
        }

        if (!PUSH_NEXT(dst, size, store, minrun, run_stack, &stack_curr,
                       &curr)) {
            kfree(run_stack);
            return;
        }
...
static __inline void HEAP_SIFT_DOWN(SORT_TYPE *dst,
                                    const size_t start,
                                    const size_t end)
{
    size_t root = start;
...
/* arr[M..-1] - kfree, arr[0,L1-1]++arr[L1,L1+L2-1] -> arr[M,M+L1+L2-1] */
static void SQRT_SORT_MERGE_LEFT_WITH_X_BUF(SORT_TYPE *arr,
                                            int L1,
                                            int L2,
                                            int M)
{
...
static void SQRT_SORT_MERGE_DOWN(SORT_TYPE *arr,
                                 SORT_TYPE *arr2,
                                 int L1,
                                 int L2)
{
...
static void SQRT_SORT_SMART_MERGE_WITH_X_BUF(SORT_TYPE *arr,
                                             int *alen1,
                                             int *atype,
                                             int len2,
                                             int lkeys)
...
/*
  ...
  is possible.
*/
static void SQRT_SORT_MERGE_BUFFERS_LEFT_WITH_X_BUF(int *keys,
                                                    int midkey,
                                                    SORT_TYPE *arr,
                                                    int nblock,
                                                    int lblock,
                                                    ...
                                                    int llast)
{
    int prest, lrest, frest, pidx, cidx;
    ...
    for (cidx = 1; cidx < nblock; cidx++, pidx += lblock) {
        prest = pidx - lrest;
        int fnext = keys[cidx] < midkey ? 0 : 1;
        ...
        if (fnext == frest) {
            ...
            SQRT_SORT_SMART_MERGE_WITH_X_BUF(arr + prest, &lrest, &frest,
                                             lblock, lblock);
        }
    }
...
*/
static void SQRT_SORT_COMBINE_BLOCKS(SORT_TYPE *arr,
                                     int len,
                                     int LL,
                                     int lblock,
                                     int *tags)
{
    int M, b, lrest, i, p, v, kc;
    M = len / (2 * LL);
    lrest = len % (2 * LL);
    ...
    for (b = 0; b <= M; b++) {
        ...
        for (u = 1; u < NBlk; u++) {
            ...
            if (p != u - 1) {
                SQRT_SORT_SWAP_N(arr1 + (u - 1) * lblock, arr1 + p * lblock,
                                 lblock);
                i = tags[u - 1];
                tags[u - 1] = tags[p];
                tags[p] = i;
        ...
        if (llast != 0) {
            while (nbl2 < NBlk &&
                   SORT_CMP_A(arr1 + NBlk * lblock,
                              arr1 + (NBlk - nbl2 - 1) * lblock) < 0) {
                nbl2++;
            }
        }

        SQRT_SORT_MERGE_BUFFERS_LEFT_WITH_X_BUF(tags, midkey, arr1, NBlk - nbl2,
                                                lblock, nbl2, llast);
    }
...
static void SQRT_SORT_COMMON_SORT(SORT_TYPE *arr,
                                  int Len,
                                  SORT_TYPE *extbuf,
                                  int *Tags)
{
    ...
    while (Len > (cbuf *= 2)) {
        SQRT_SORT_COMBINE_BLOCKS(arr + lblock, Len - lblock, cbuf, lblock,
                                 Tags);
    }
...
void SQRT_SORT(SORT_TYPE *arr, size_t Len)
{
    ...
    Tags = (int *) kmalloc(NK * sizeof(int), GFP_KERNEL);
...
static int GRAIL_FIND_KEYS(SORT_TYPE *arr, int len, int nkeys)
{
    ...
    while (u < len && h < nkeys) {
        int r = GRAIL_BIN_SEARCH_LEFT(arr + h0, h, arr + u);
...
static void GRAIL_SMART_MERGE_WITH_BUFFER(SORT_TYPE *arr,
                                          int *alen1,
                                          int *atype,
                                          int len2,
                                          int lkeys)
...
static void GRAIL_SMART_MERGE_WITHOUT_BUFFER(SORT_TYPE *arr,
                                             int *alen1,
                                             int *atype,
                                             int _len2)
{
    ...
    if (len1 && SORT_CMP_A(arr + (len1 - 1), arr + len1) - ftype >= 0) {
        while (len1) {
            int h = ftype ? GRAIL_BIN_SEARCH_LEFT(arr + len1, len2, arr)
...
static void GRAIL_SMART_MERGE_WITH_X_BUF(SORT_TYPE *arr,
                                         int *alen1,
                                         int *atype,
                                         int len2,
                                         int lkeys)
                                                ...
                                                int llast)
{
    ...
    for (cidx = 1; cidx < nblock; cidx++, pidx += lblock) {
        prest = pidx - lrest;
        int fnext = SORT_CMP_A(keys + cidx, midkey) < 0 ? 0 : 1;
        ...
        if (fnext == frest) {
            ...
            GRAIL_SMART_MERGE_WITH_X_BUF(arr + prest, &lrest, &frest, lblock,
                                         lblock);
        }
    }
...
/*
  ...
  output: first K elements are buffer, blocks 2*K and last subblock sorted
*/
static void GRAIL_BUILD_BLOCKS(SORT_TYPE *arr,
                               int L,
                               int K,
                               SORT_TYPE *extbuf,
                               int LExtBuf)
...
/*
  ...
  is possible.
*/
static void GRAIL_MERGE_BUFFERS_LEFT(SORT_TYPE *keys,
                                     SORT_TYPE *midkey,
                                     SORT_TYPE *arr,
                                     int nblock,
                                     int lblock,
                                     ...
                                     int llast)
{
    ...
    for (cidx = 1; cidx < nblock; cidx++, pidx += lblock) {
        ...
        if (fnext == frest) {
            ...
            if (havebuf) {
                GRAIL_SMART_MERGE_WITH_BUFFER(arr + prest, &lrest, &frest,
                                              lblock, lblock);
            } else {
                GRAIL_SMART_MERGE_WITHOUT_BUFFER(arr + prest, &lrest, &frest,
                                                 lblock);
            }
        }
    }
...
/*
  ...
  in pairs LL and nkeys are powers of 2. (2*LL/lblock) keys are guarantied
*/
static void GRAIL_COMBINE_BLOCKS(SORT_TYPE *keys,
                                 SORT_TYPE *arr,
                                 int len,
                                 int LL,
                                 int lblock,
                                 ...
                                 SORT_TYPE *xbuf)
{
    ...
    for (b = 0; b <= M; b++) {
        ...
        for (u = 1; u < NBlk; u++) {
            ...
            if (p != u - 1) {
                GRAIL_SWAP_N(arr1 + (u - 1) * lblock, arr1 + p * lblock,
                             lblock);
        ...
        if (xbuf) {
            ...
            GRAIL_MERGE_BUFFERS_LEFT(keys, keys + midkey, arr1, NBlk - nbl2,
                                     lblock, havebuf, nbl2, llast);
        }
    }
...
static void GRAIL_COMMON_SORT(SORT_TYPE *arr,
                              int Len,
                              SORT_TYPE *extbuf,
                              int LExtBuf)
{
    int lblock, nkeys, findkeys, ptr, cbuf, nk;
    int havebuf;
    long long s;
...
void GRAIL_SORT_FIXED_BUFFER(SORT_TYPE *arr, size_t Len)
{
    ...
    SORT_TYPE *ExtBuf =
        kmalloc(sizeof(SORT_TYPE) * GRAIL_EXT_BUFFER_LENGTH, GFP_KERNEL);
